...
weekly page. Netflix's per-country page carries the platform's own official ranking, which
carries no language split, so `language` is ignored there. Both impossible pairings
(`platform: "amazon-prime"` with a country, and any `language` on a country entry) only warn
at config validation — the entry is skipped, never a hard error.
